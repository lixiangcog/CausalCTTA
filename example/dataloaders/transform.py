...
def collate_fn_wo_transform(batch):
    image, label, pseudo ,name = zip(*batch)
    image = np.stack(image, 0)
    label = np.stack(label, 0)
    pseudo = np.stack(pseudo, 0)
    name = np.stack(name, 0)
    data_dict = {'data': image, 'mask': label, 'pseudo_label': pseudo, 'name': name}
    data_dict['mask'] = to_one_hot_list(data_dict['mask'])
    data_dict['pseudo_label'] = to_one_hot_list(data_dict['pseudo_label'])
    return data_dict
