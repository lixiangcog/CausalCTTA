...
class DropResUnet(nn.Module):
    def __init__(self, resnet='resnet34', num_classes=2, pretrained=False, convert=True, newBN=AdaBN, warm_n=5,dropout_config=None):
        super().__init__()
        if resnet == 'resnet34':
            base_model = resnet34
            bottleneck = False
        ...
        # 新增：Dropout层
        self.bottleneck_dropout = nn.Dropout2d(dropout_config['bottleneck'])
        self.up1_dropout = nn.Dropout2d(dropout_config['decoder_mid'])
        self.up2_dropout = nn.Dropout2d(dropout_config['decoder_mid'])
        self.up3_dropout = nn.Dropout2d(dropout_config['decoder_late'])
        self.up4_dropout = nn.Dropout2d(dropout_config['decoder_late'])
        self.pre_seg_dropout = nn.Dropout2d(dropout_config['pre_final'])
    ...
    def forward(self, x):
        ...
        x = self.up1(x, sfs[3])
        x = self.up1_dropout(x)
        x = self.up2(x, sfs[2])
        x = self.up2_dropout(x)
        x = self.up3(x, sfs[1])
        x = self.up3_dropout(x)
        x = self.up4(x, sfs[0])
        x = self.up4_dropout(x)
        x = self.up5(x)
        head_input = F.relu(self.bnout(x))
        head_input = self.pre_seg_dropout(head_input)
