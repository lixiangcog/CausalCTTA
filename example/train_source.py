...
from config import *
from networks.origin_ResUnet import ResUnet
...
class ClassErrorMeter(object):
    ...
    def add(self, output, target):
        ...
        # 计算 top-k 准确率
        maxk = max(self.topk)
        pred = torch.from_numpy(output).topk(maxk, 1, True, True)[1].t()
        correct = pred.eq(torch.from_numpy(target).view(1, -1).expand_as(pred))
        
        for k in self.topk:
            correct_k = correct[:k].reshape(-1).float().sum(0).item()
            self.sum_dict[k] += correct_k

    def value(self):
        return [100 * (1 - self.sum_dict[k] / self.n) for k in self.topk]

    def reset(self):
        self.n = 0
        self.sum_dict = {k: 0 for k in self.topk}
...
class TrainSource:
    ...
    def build_model(self):
        ...
        checkpoint = torch.load(os.path.join(self.model_path, 'last-Res_Unet.pth'))
        self.model.load_state_dict(checkpoint, strict=True)
        #vida_params, replaced_names,self.model = replace_1x1_conv_with_vida(self.model)
        if self.optim == 'SGD':
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_root', type=str, default='./OPTIC/Fundus/')
